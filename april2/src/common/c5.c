...
static inline void uc5_copy(struct uc5_state *state)
{
    ...
    if (len >= 10) {
        ...
        // we've now written 8 bytes of the pattern.
...
void uc5(const uint8_t *_in, int _inlen, uint8_t *_out, int *_outlen)
{
    ...
    uint32_t encoded_outlen = uc5_length(_in, _inlen);
    ...
    *_outlen = state->outpos;
    ...
    assert(encoded_outlen == state->outpos);
}
...
{
    // our goal is to produce ~16 bits of "entropy" from three bytes so
...
static inline void c5_update_history(struct c5_state *state, int inpos)
{
    ...
    // NB: the line below can occasionally trigger a "use of
    // uninitialized value" warning with valgrind when computing the
    // key reads beyond the input buffer. But this is inconsequential
    // for correctness, since a "bad" key value will just result in a
