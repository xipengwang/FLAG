...
/////////////////////////////////////////////////////////////////////////////////////////
// XYZ Factor
static april_graph_factor_t* xyz_factor_copy(april_graph_factor_t *factor)
{
...
static april_graph_factor_eval_t* xyz_factor_eval(april_graph_factor_t *factor, april_graph_t *graph, april_graph_factor_eval_t *eval)
{
    if (eval == NULL) {
    ...
    double xa = na->state[0], ya = na->state[1], za = na->state[2];
    double xb = nb->state[0], yb = nb->state[1], zb = nb->state[2];
    ...
    eval->r[2] = factor->u.common.z[2] - zhat[2];
...
static void xyz_factor_destroy(april_graph_factor_t *factor)
{
    free(factor->nodes);
...
static void april_graph_factor_xyz_encode(const stype_t *stype, uint8_t *data, uint32_t *datapos, const void *obj)
{
...
static void *april_graph_factor_xyz_decode(const stype_t *stype, const uint8_t *data, uint32_t *datapos, uint32_t datalen)
{
    ...
    april_graph_factor_t *factor = april_graph_factor_xyz_create(a, b, z, ztruth, W);
...
const stype_t stype_april_factor_xyz = { .name = "april_graph_factor_xyz",
                                         .encode = april_graph_factor_xyz_encode,
                                         .decode = april_graph_factor_xyz_decode,
                                         .copy = NULL };
...
april_graph_factor_t *april_graph_factor_xyz_create(int a, int b, const double *z, const double *ztruth, const matd_t *W)
{
    ...
    factor->stype = &stype_april_factor_xyz;
    factor->type = APRIL_GRAPH_FACTOR_XYZ_TYPE;
    factor->nnodes = 2;
    ...
    factor->copy = xyz_factor_copy;
    factor->eval = xyz_factor_eval;
    factor->destroy = xyz_factor_destroy;
...
static void xyz_node_update(april_graph_node_t *node, double *dstate)
{
    for (int i = 0; i < 3; i++)
...
static april_graph_node_t* xyz_node_copy(april_graph_node_t *node)
{
    assert(0);
    return NULL;
}

static void xyz_node_destroy(april_graph_node_t *node)
{
    free(node->state);
    free(node->init);
    free(node->truth);
...
static void april_graph_node_xyz_encode(const stype_t *stype, uint8_t *data, uint32_t *datapos, const void *obj)
{
...
static void *april_graph_node_xyz_decode(const stype_t *stype, const uint8_t *data, uint32_t *datapos, uint32_t datalen)
{
    double state[3];
    ...
    april_graph_node_t *node = april_graph_node_xyz_create(state, init, truth);
...
const stype_t stype_april_node_xyz = { .name = "april_graph_node_xyz",
                                       .encode = april_graph_node_xyz_encode,
                                       .decode = april_graph_node_xyz_decode,
                                       .copy = NULL };
...
april_graph_node_t *april_graph_node_xyz_create(const double *state, const double *init, const double *truth)
{
    ...
    node->type = APRIL_GRAPH_NODE_XYZ_TYPE;
    node->length = 3;
    node->state = doubles_dup(state, 3);
    node->init = doubles_dup(init, 3);
    node->truth = doubles_dup(truth, 3);

    node->update = xyz_node_update;
    node->copy = xyz_node_copy;
    node->destroy = xyz_node_destroy;

    node->stype = &stype_april_node_xyz;
    return node;
}

void april_graph_xyz_stype_init()
{
    stype_register(&stype_april_node_xyz);
    stype_register(&stype_april_factor_xyz);
}
