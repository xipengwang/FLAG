...
void image_source_pgusb_enumerate(zarray_t *urls)
{
    libusb_context *context;
...
//        if (transfer->actual_length == 512) {
            ...
            if (impl->current_frame_offset != 0) {
                ...
                static int sync_failures = 0;
                sync_failures++;
                assert(sync_failures < 100);
            }
...
void image_source_pgusb_setup()
{
   struct image_source_provider *provider = calloc(1, sizeof(struct image_source_provider));
   provider->protocol = strdup("pgusb://");
   provider->open = image_source_pgusb_open;
   provider->enumerate = image_source_pgusb_enumerate;
