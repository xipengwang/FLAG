...
static image_source_t *image_source_null_open(url_parser_t *urlp)
{
...
static void image_source_null_enumerate(zarray_t *urls)
{
    char *p = strdup("null://");
    zarray_add(urls, &p);
}

void image_source_null_setup()
{
   struct image_source_provider *provider = calloc(1, sizeof(struct image_source_provider));
   provider->protocol = strdup("null://");
   provider->open = image_source_null_open;
   provider->enumerate = image_source_null_enumerate;
