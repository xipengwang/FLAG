...
static void *reader_thread(void *arg)
{
    ...
    while (1) {
		...
		int fd = tcp_util_connect(impl->hostname, impl->port);
		if (fd < 0)
		   goto error;
      ...
      error:
		...
		close(fd); // XXX does fdopen dup()? double close?
        fclose(f);
        sleep(1);
    }
...
void image_source_tcp_setup()
{
   struct image_source_provider *provider = calloc(1, sizeof(struct image_source_provider));
   provider->protocol = strdup("tcp://");
   provider->open = image_source_tcp_open;
