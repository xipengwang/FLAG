...
static void image_source_v4l2_enumerate(zarray_t *urls)
{
    for (int i = 0; i < 16; i++) {
        char buf[1024];
...
void image_source_v4l2_setup()
{
   struct image_source_provider *provider = calloc(1, sizeof(struct image_source_provider));
   provider->protocol = strdup("v4l2://");
   provider->open = image_source_v4l2_open;
   provider->enumerate = image_source_v4l2_enumerate;
