...
vx_object_t *vxo_robot_solid(float rgba[4])
{
    ...
    if (program_resource == NULL) {
        ...
        float verts[] = { -DEFAULT_LENGTH/2,   DEFAULT_WIDTH/2,
                          -DEFAULT_LENGTH/2,  -DEFAULT_WIDTH/2,
                          DEFAULT_LENGTH/2,    0.0f
        };

        vertex_resource = vx_resource_make_attr_f32_copy(verts, 6, 2);
    ...
    return vxo_generic_create(program_resource,
                              ...
                              (struct vxo_generic_draw[]) {
                                  { .command = VX_GL_TRIANGLES, .first = 0, .count = 3 },
                                  { .count = 0 }, });
}


vx_object_t *vxo_robot_line(float rgba[4], float lineWidth)
{
    static vx_resource_t *program_resource = NULL, *vertex_resource = NULL;
    ...
    return vxo_generic_create(program_resource,
                              ...
                              (struct vxo_generic_draw[]) {
                                  { .command = VX_GL_LINE_STRIP, .first = 0,
                                    .count = vertex_resource->u.attr_f32.nelements / vertex_resource->u.attr_f32.dim },
                                  { .count = 0 }, });
