...
int main(int argc, char *argv[])
{
    ...
    sprintf(vectors_path, "%s/%s", path, "rsa.vectors");
    ...
    while (NULL != fgets(_buf, sizeof(_buf), f)) {
        ...
        if (str_starts_with(buf, "# Encryption")) {
            char *s = read_ascii_block(f);
            free(encryption);
            ...
            if (1) {
                enc_usecs -= utime_now();
                ek_rsa_pkcs_15_encrypt(modulus, pubexp, seed, message, messagelen, enc, &enc_len);
                enc_usecs += utime_now();
            }
            ...
            if (1) {
                dec_privexp_usecs -= utime_now();
                if (ek_rsa_pkcs_15_decrypt_privexp(modulus, privexp, enc, msg, &msglen)) {
                    printf("DEC-EXP FAIL\n");
                    exit(0);
                }
                dec_privexp_usecs += utime_now();
            ...
            if (1) {
                dec_crt_usecs -= utime_now();

                if (ek_rsa_pkcs_15_decrypt_crt(modulus,
                                               prime1, prime2,
                                               primeexp1, primeexp2,
                                               coeff,
                                               enc, msg, &msglen)) {
            ...
            assert(msglen == messagelen);

            if (!memcmp(msg, message, messagelen))
                printf("DEC OK\n");
            else {
                assert(0);
            }
